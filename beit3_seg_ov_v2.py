...
class BEiT3SegTransformerModule(nn.Module):
    ...
    def forward(
        ...
        if text_features is not None and self.use_text_features:
            ...
            # text_pos_embedding = self.text_position_embedding.weight[:text_seq_len].unsqueeze(1).repeat(1, batch_size, 1)
            # text_embeddings = text_embeddings + text_pos_embedding
        else:
...
class BEiT3SegForUniversalSegmentation(Mask2FormerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if self.use_objectness_loss and self.use_text_contrastive_loss:
            ...
            self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07), requires_grad=True)
            # self.null_embed = nn.Parameter(torch.randn(1, 1, config.hidden_dim))
            # # self.target_embedding = nn.Embedding(config.num_labels+1, config.hidden_dim)
    ...
    def forward(
        ...
        # cls loss use text out
        # for decoder_output in outputs.transformer_decoder_text_intermediate_states:
        #     class_prediction = self.class_predictor(decoder_output.transpose(0, 1))
        #     class_queries_logits += (class_prediction,)
        ...
        # print(len(class_queries_logits), len(masks_queries_logits))
